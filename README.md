# Blog-de-noticias

Un proyecto de un Blog de noticias enfocado a la Tecnología Agrícola y su desarrollo. El objetivo es proporcionar una plataforma robusta para la publicación, gestión y lectura de artículos de noticias sobre todo lo relacionado a este rubro y su amplio campo.

Está diseñado en Python con Django para la lógica del blog, SQLite3 para la gestión de la base de datos, y también cuenta con un sistema de autenticación de usuarios. Se creo un Grupo COLABORADORES que son los unicos que pueden grabar Publicaciones. El apartado visual está diseñado con HTML y CSS para la estructura visual de los elementos del blog.


Integrantes del Proyecto: Franco Riquelme, Juan de Dios Candia, Tiago Dominguez, Santiago Portillo.
